...
function createTask(task, taskName) {
    if (task.type === 'concat') {
        (function (task, taskName) {
            ...
            return gulp.task(taskName, gulp.series(cleanTaskName, runTaskName, function () {
                browserSync.reload();
            }))
        })(task, taskName)
    }
        ...
        (function (task, taskName) {
            ...
            return gulp.task(taskName, gulp.series(cleanTaskName, function () {
        ...
        (function (task, taskName) {
            ...
            gulp.task(taskName, gulp.series(cleanTaskName, runTaskName, function () {
                browserSync.reload();
            }))
        })(task, taskName)
...
let config = require('./gulp/gulp.config')(isProduction, buildPath + routeBase);

for (let key in config["base"]) {
    let task = config["base"][key];
    let taskName = "base." + key;
    createTask(task, taskName);
    gulp.watch(task.watch || task.src, gulp.series(taskName));
    tasks.push(taskName);
}

for (let key in config["prod"]) {
    let task = config["prod"][key],
        taskName = key;
    createTask(task, key);
}
...
gulp.task('clean', function () {
    return gulp.src(resourcePath, {allowEmpty: true}).pipe(plugins.clean({ force: true }));
});
gulp.task('clean-assets', function () {
    return gulp.src(resourcePath + 'assets/', {allowEmpty: true}).pipe(plugins.clean({ force: true }));
});
