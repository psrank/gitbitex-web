...
export class TradeStore extends Store {
  ...
  loadProductHistory(
    ...
    HttpService.Trade.getProductHistory(productId, granularity).then(
      (response: any) => {
  ...
  loadTradeHistory(productId: string) {
    HttpService.Trade.getProductTradeHistory(productId).then(
      (response: any) => {
        response.forEach((trade: any) => {
          trade.localTime = Moment(trade.time).format("H:mm:ss");
        });
        this.store.commit("setTradeHistory", {
          productId: productId,
          trades: response
        });
      }
  ...
  loadOpenOrders(productId: string, callback?: () => void) {
    ...
    HttpService.Order.getOrders(productId, 30, ["open"]).then(
      (response: any) => {
        ...
        this.store.commit("setOpenOrders", {
          productId: productId,
  ...
  updateOrderBook(buffers: any) {
    ...
    buffers.forEach((buffer: any) => {
      ...
      changes[buffer.productId] = changes[buffer.productId].concat(
        buffer.changes
      );
    });
    ...
    for (const productId in changes) {
      ...
      changes[productId].forEach((change: any) => {
        if (change[0] == "buy") {
          change[2] == 0
            ? delete bids[change[1]]
            : (bids[change[1]] = [change[1], change[2]]);
        } else {
          change[2] == 0
            ? delete asks[change[1]]
  ...
  updatePruductPrice(data?: any) {
    ...
    this.products.forEach((product: any) => {
      if (product.id == data.productId) {
        ...
        product.rate24h = (
          (product.price * 100) / product.open24h -
          100
        ).toFixed(2);
  ...
  parseWebSocketMessage(msg: any) {
    if (msg.type == "snapshot") {
      ...
      this.bookBuffer ||
        (this.bookBuffer = new SocketMsgBuffer((buffers: any[]) => {
      ...
      this.orderBuffer ||
        (this.orderBuffer = new SocketMsgBuffer((buffers: any[]) => {
          this.updateOrder(buffers);
        }, 300));
      this.orderBuffer.push(msg);
    } else if (msg.type == "funds") {
      this.fundsBuffer ||
        (this.fundsBuffer = new SocketMsgBuffer((buffers: any[]) => {
