...
export class Helper {
  ...
  static Trade_margeOrderBook(
    ...
    if (count && bids.length > 0 && asks.length > 0) {
      ...
      for (let i = 0; bids.length < count; i++) {
        bids.push([
          this.Trade_scalePrice(lastBidPrice - (i + 1) * scale, scale),
          0
        ]);
      }
      ...
      for (let i = 0; asks.length < count; i++) {
        asks.push([
          this.Trade_scalePrice(lastAskPrice + (i + 1) * scale, scale),
          0
        ]);
      }
    }
  ...
  static Trade_scalePrice(price: number, scale: number) {
    ...
    return scale >= 1
      ? String(price)
      : price.toFixed(this.Number_toString(scale).length - 2);
  }
